...
def get_pixels_point_location_as_pandas(tile_path, combined_trees):
    #mask_path
    mask = cv2.imread(CONST_DIR_INNER_DATA + 'coustline_tiles_masks\\' + os.path.basename(tile_path))[:,:,0]
    combined_trees = np.where((mask == 0) & (combined_trees == 1), 1, 0)
...
def save_grass_object(y_pred, tile_path):
    ...
    #mask_path
    mask_between_areas = cv2.imread(CONST_DIR_INNER_DATA + 'coustline_tiles_masks\\' + os.path.basename(tile_path))[:,:,0]
    mask = np.where((mask_between_areas == 0) & (mask == 1), 1, 0)
...
def worker_function(target):
    ...
    with open(CONST_DIR_INNER_DATA + 'tiles_between_areas.pkl', 'rb') as f:
...
if __name__ == '__main__':
    if (c['tiles_between_areas']['analyze']):
        # Read the shapefile containing multiple polygons (seas)
        multipolygons = gpd.read_file(c['path']['shapefile_of_non_tree_areas'])
        ...
        def make_masks_of_between_areas():
            ...
            directory = CONST_DIR_INNER_DATA + 'coustline_tiles_masks\\'
